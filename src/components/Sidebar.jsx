  ...
  return (
    <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''}`} aria-expanded={!isCollapsed}>
      <div className="sidebar-header">
        <Link to="/" className="sidebar-logo" style={{ textDecoration: 'none' }} aria-label="ArenaPulse Home">
          <Radar className="logo-icon animate-pulse-glow" size={28} aria-hidden="true" />
          {!isCollapsed && <span className="logo-text">ArenaPulse</span>}
        </Link>
        <button className="menu-btn" onClick={() => setIsCollapsed(!isCollapsed)} style={{ padding: '0.5rem', color: '#fff' }} aria-label="Toggle Sidebar">
          {/* Hamburger Menu Icon */}
